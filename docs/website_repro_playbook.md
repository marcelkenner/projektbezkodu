...
9. Run `npm run content:lint` (automatically executed before `npm run build`) to validate every markdown file with `gray-matter`. Fix YAML errors before committing so the global content crawl never fails.
   - YAML gotcha: if a scalar value contains `: ` (colon + space), it must be quoted (example: `title: "FAQ: Depositphotos"`), otherwise `gray-matter` / `js-yaml` will fail parsing.
   ...
   - The linter now also enforces production route families: tutorials must stay under `/poradniki/`, comparisons under `/porownania/`, resources under `/zasoby/`, templates under `/szablony/`, glossary terms under `/glossary/`, lead magnets under `/do-pobrania/`, and tool guides under `/narzedzia/<tool>/...`.
   ...
   - Use `npm run content:public:paths` to repair explicit path mistakes in those route families without touching `_examples` fixtures.
...
11. `/artykuly` aggregates published markdown whose canonical `path` lives under `/artykuly/.../` and whose frontmatter `type` is not `hub`. Keep `path` inside the matching category prefix (for example files under `content/artykuly/seo/**` should publish under `/artykuly/seo/...`) because the listing page, breadcrumbs, hub pages, and sitemap depend on that contract.
12. Article hubs live under `/artykuly/<kategoria>/` and `/artykuly/<kategoria>/<podkategoria>/`: these pages **do not render markdown bodies**. They render:
   ...
   - The visible hub heading and intro copy from the hub frontmatter.
...
14. `/artykuly/<slug>/` dispatches:
   ...
   - Category root `index.md` files are reserved for hubs. Direct leaf articles inside a category must use a deeper URL such as `/artykuly/<kategoria>/<artykul>/`.
...
19. Artykuły routing: you can store multiple markdown pages under `content/artykuly/**` (including multiple `index.md` files at different depths). Category `index.md` files are reserved for hubs. Subcategory `index.md` files become hubs when published article pages exist below them. `npm run format` runs `node scripts/fix-artykuly-paths.mjs` to backfill missing `slug`/`title` and keep canonical paths under the matching `/artykuly/<kategoria>/...` prefix; `npm run content:lint` enforces the hub hierarchy and the category-prefix rule before build. See `docs/frontmatter_and_routing.md`.
...
3. Create listing pages:
   ...
   - Comparisons (`app/(marketing)/(content)/porownania/page.tsx`) and search (`app/(marketing)/(content)/szukaj/page.tsx`) backed by `ContentSearchEngine`. Search must cover every public markdown-backed content route plus glossary entries, not only the original article/tutorial/comparison subset.
4. Wire featured article categories into navigation (copy-driven); the footer auto-lists only published article hubs found under `content/artykuly` and lays them out in a two-row grid.
...
6. Create dynamic detail routes `[slug]/page.tsx` for each content type with:
	   ...
	   - Article-style hero images rendered by `app/ui/heroes/ContentHero.tsx` must stay in the initial HTML, use explicit `fetchPriority="high"` + `loading="eager"`, and keep stable `width`, `height`, and `sizes` values so mobile LCP is easy for the browser to discover.
...
7. Tool content convention: store tool metadata + main guide content at `content/narzedzia-no-code/<tool>/glowny/index.md` and every tool-specific article under `content/narzedzia-no-code/<tool>/**/index.md` (any depth). The `/narzedzia/<tool>/` page renders only the recursive `ArticleGrid`, while the main guide is readable at `/narzedzia/<tool>/glowny/`.
   Legacy top-level tool folders such as `content/instapage/**` still need explicit canonicals under `/narzedzia/<tool>/...`; they are not allowed to publish at bare paths like `/faq/` or `/recenzja/`.
...
4. XML sitemap (`app/sitemap.ts`) is built by `SitemapGenerator`, which aggregates App Router static pages, ContentLibrary routes, and dynamic directories (glossary, tags, authors, lead magnets, templates). `/mapa-strony/` permanently redirects to `/sitemap.xml` so the XML feed stays authoritative.
...
2. Hero (`HeroSection.tsx`): headline, supporting copy, dual CTA (Webflow + plan) with microcopy disclosure, responsive hero image (`/img/hero-3x2*.webp`). Homepage CTA targets must use published routes such as `/narzedzia/webflow/`, `/artykuly/sciezki/webflow-30-dni/`, and `/artykuly/automation/`.
...
2. Article detail (`/artykuly/**` matching an article `path`): show breadcrumbs, meta (reading time, publish, update, author), the affiliate disclosure, sticky TOC, “Next step” CTA block, author card, and related articles.
   - Shared header wordmarks in `app/(marketing)/layout.tsx` and `app/(legal)/layout.tsx` must not use `priority`; reserve high-priority image loading for the actual page hero/LCP candidate.
...
1. Resources (`/zasoby`, `/zasoby/[slug]`): rely on `ResourceRepository` + `ResourceDirectory` with format/topic/time filters; detail pages use the `resource-detail` layout with file metadata and a download CTA.
   - If a resource, lead magnet, or case-study page renders its hero with a native `<img>`, that image must also use `fetchPriority="high"` + `loading="eager"` when it appears above the fold.
