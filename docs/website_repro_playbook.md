...
13. Default hero fallbacks: `/artykuly` → `/img/articles_hero_image.jpeg`, `/poradniki` → `/img/tutorials_hero_image.jpeg`, `/porownania` → `/img/comparisons_hero_image.jpeg`, `/szablony` → `/img/templates_hero_image.jpeg` (wired via `heroImageResolver.defaultHeroImage`). Use these assets when front matter lacks an explicit hero.
