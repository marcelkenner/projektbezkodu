...
class HeroImageEnforcer {
  ...
  async processFile(filePath) {
    ...
    const nextData = this.applyHeroImage(parsed.data);
  ...
  applyHeroImage(frontmatter) {
    const next = { ...frontmatter };
    const currentHero = this.normalizeHero(next.hero);
    next.hero = {
      ...currentHero,
      image: {
  ...
  printSummary(totalFiles) {
    ...
    console.log(
      `[set-narzedzia-hero] Scanned ${totalFiles} markdown file(s) under ${relativeRoot}.`,
    );
    console.log(
      `[set-narzedzia-hero] Updated hero image in ${this.updatedCount} file(s).`,
    );
    if (this.failedFiles.length > 0) {
      console.error(
        "[set-narzedzia-hero] Failed to parse front matter for the file(s) below:",
      );
...
async function main() {
  const enforcer = new HeroImageEnforcer(CONTENT_ROOT);
  await enforcer.run();
}

main().catch((error) => {
  console.error("[set-narzedzia-hero] Unexpected error.");
  console.error(error);
  process.exitCode = 1;
});
