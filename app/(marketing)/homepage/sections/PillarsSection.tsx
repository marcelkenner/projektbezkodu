...
import { HomepageIconFactory, type PillarIconName } from "./HomepageIconFactory";
...
export function PillarsSection({ copy }: { copy: PillarsCopy }) {
  ...
  return (
    <section className="homepage-section homepage-section--pillars" aria-labelledby="pillars-heading">
      <div className="pbk-container">
        <h2 id="pillars-heading">{copy.heading}</h2>
        <div className="homepage-pillars__grid">
          {copy.items.map((item) => (
            <Link key={item.title} href={item.href} className="homepage-pillars__card">
