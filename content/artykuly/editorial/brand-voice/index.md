---
title: Ton marki (brand voice) – praktyczny przewodnik z ćwiczeniami
slug: brand-voice
path: /artykuly/editorial/brand-voice/
type: guide
tags:
  - evergreen
  - content
  - brand
draft: true
date: "2025-11-05"
hero:
  heading: Ton marki (brand voice) – praktyczny przewodnik z ćwiczeniami
  subheading: Wpis roboczy — uzupełnij krótki opis, żeby nagłówek nie był pusty.
---

# Ton marki (brand voice) – praktyczny przewodnik z ćwiczeniami

Logo można zmienić w weekend.  
Ton marki – to, _jak_ mówisz w tekstach, prezentacjach, mailach – wchodzi ludziom w głowę dużo głębiej i na dłużej. :contentReference[oaicite:5]{index=5}

Ten przewodnik pomoże Ci:

- nazwać **osobowość marki** w normalnym języku, a nie „powerful, innovative, dynamic”,
- przełożyć to na konkretne zasady pisania,
- dać zespołowi ćwiczenia, po których naprawdę piszą spójniej.

---

## 1. Voice vs tone – proste rozróżnienie

- **Voice (głos)** – to, _kim_ jesteś jako marka. Raczej stały.
  - np. „bezpośredni, konkretny, nie udajemy korpo”.

- **Tone (ton)** – to, _jak_ brzmisz w danej sytuacji.
  - ta sama osoba inaczej mówi na scenie, inaczej 1:1 z klientem.

Przykład: Mailchimp ma spójny, rozmowny voice, ale ton zmienia w zależności od tego, czy gratuluje kampanii, czy tłumaczy błąd w systemie. :contentReference[oaicite:6]{index=6}

---

## 2. Ćwiczenie: 3 marki-ludzie

Weź kartkę / Notion i odpowiedz:

1. **Gdyby Twoja marka była osobą, to…**
   - ile ma lat (mentalnie),
   - czym się zajmuje,
   - jak mówi (dość konkretnie: „pół żartem, pół serio”, „spokojnie i logicznie”).

2. **3 marki / osoby, które lubisz za sposób komunikacji**
   - co dokładnie Cię w nich kręci (konkretne cechy),
   - co byś _nie_ chciał kopiować.

3. **Czego na pewno NIE chcesz w swoim voice**
   - np. „marketingowe lanie wody”, „żarty na siłę”, „motywacyjne gadki”.

To ćwiczenie nie zastąpi całego style guide’u, ale daje dobry start do rozmowy w zespole.

---

## 3. 4 wymiary tonu (spektrum)

Uproszczony model NNG dzieli ton na cztery wymiary: formalny–nieformalny, poważny–zabawny, respektujący–zadziorny, suchy–entuzjastyczny. :contentReference[oaicite:7]{index=7}

Dla każdego wymiaru zaznacz, gdzie chcesz być:

1. **Formalny ↔ Nieformalny**
2. **Poważny ↔ Z humorem**
3. **Mocno ułożony ↔ Zadziorny / luzacki**
4. **Suchy ↔ Entuzjastyczny**

Przykład (marka: konsultant analityki dla małych firm):

- formalny/nieformalny: **trochę po środku, lekko w stronę nieformalnego, ale bez slangu**,
- poważny/z humorem: **raczej poważnie, z okazjonalnym, suchym żartem**,
- ułożony/zadziorny: **konkretny, czasem dosadny przy obalaniu mitów**,
- suchy/entuzjastyczny: **bardziej rzeczowy niż „wow, super projekt!”**.

Zapisz to w formie tabelki dla całego zespołu.

---

## 4. Wartości → cechy głosu

Teraz przełóż wartości na **konkretne cechy językowe**.

### Krok 1: spisz 3–5 wartości

Np.:

- szczerość,
- praktyczność,
- szacunek do czasu klienta,
- partnerstwo.

### Krok 2: przetłumacz je na język tekstów

Przykład:

- **Szczerość** → mówimy, kiedy coś _nie_ jest dla klienta, nie obiecujemy „10x w miesiąc”.
- **Praktyczność** → każdy tekst ma wskazówki do zastosowania od razu, nie tylko „inspiracje”.
- **Szacunek do czasu** → krótkie leady, zero lania wody, jasno oznaczone podsumowania.

Spisz to w formie:

| Wartość        | Co to znaczy w tekstach                                |
| -------------- | ------------------------------------------------------ |
| szczerość      | mówimy wprost o ograniczeniach, nie obiecujemy cudów   |
| praktyczność   | w każdym tekście przynajmniej 1 przykład lub template  |
| szacunek czasu | nie piszemy „esejów” bez struktury, dbamy o czytelność |

---

## 5. Tabela „tak piszemy / tak nie piszemy”

To najbardziej przyziemna, ale przydatna część.

Zrób tabelę:

| Aspekt              | Tak piszemy                           | Tak NIE piszemy                                 |
| ------------------- | ------------------------------------- | ----------------------------------------------- |
| Zwrot do czytelnika | „Ty”, „Wy”, w 2. osobie               | bezosobowe „należy”, „trzeba”                   |
| O naszym produkcie  | konkretnie: „kurs analityki”, „audyt” | „innowacyjne rozwiązania transformujące biznes” |
| O błędach           | „to normalne, że…”, „większość firm…” | „robisz to źle”, „to fatalny błąd”              |
| Call to action      | „Zobacz, czy to dla Ciebie”           | „Kup teraz, zanim będzie za późno”              |

Dodaj przykłady zdań „przed / po” – najlepiej z własnych tekstów.

---

## 6. Brand voice w różnych kontekstach

Twój voice jest stały, ale **ton** różni się w zależności od sytuacji.

Zrób prostą tabelę z kontekstami i wskazówkami:

| Kontekst                  | Ton                         | Przykład                               |
| ------------------------- | --------------------------- | -------------------------------------- |
| strona główna             | klarowny, trochę entuzjasty | „Pomagamy Ci widzieć, co działa”       |
| blog ekspercki            | spokojny, rzeczowy          | „Pokażę Ci, jak to ogarnąć w praktyce” |
| maile sprzedażowe         | bezpośredni, konkretny      | „Jeśli chcesz X, zobacz tę ofertę”     |
| maile z błędami / support | empatyczny, uspokajający    | „To nasza wina, naprawiamy to tak…”    |

Możesz się inspirować tym, jak robią to duże marki (np. Duolingo adaptuje ton w zależności od kontekstu w aplikacji). :contentReference[oaicite:8]{index=8}

---

## 7. Ćwiczenie dla zespołu: „przepisywanie”

Weź 2–3 fragmenty starych tekstów (np. fragment oferty, maila, artykułu) i poproś zespół:

1. Przeczytaj fragment.
2. Zaznacz, co jest **nie w stylu** naszej marki (słowa, zwroty, konstrukcje).
3. Przepisz akapit tak, jak MÓWIŁABY to nasza marka.

Potem razem omawiacie:

- gdzie się rozjechały interpretacje,
- które rozwiązania wam się podobają,
- co dopisać do „tak piszemy / tak nie piszemy”.

Po kilku rundach ludzie zaczynają czuć voice „w palcach”, a nie tylko w dokumencie.

---

## 8. Mini-styleguide do skopiowania

**1. Kim jesteśmy jako marka (2–3 zdania):**  
…

**2. 3–5 cech głosu (z krótkim opisem):**

- …
- …
- …

**3. Spektrum tonu:**

- formalny ↔ nieformalny: …
- poważny ↔ z humorem: …
- suchy ↔ entuzjastyczny: …
- ułożony ↔ zadziorny: …

**4. Tak piszemy / tak nie piszemy:**

(tabelka z rozdziału 5)

**5. Przykłady fragmentów „w naszym stylu”:**

- fragment strony głównej,
- fragment artykułu,
- fragment maila.

Taki przewodnik nie musi mieć 40 stron. Ważne, żeby był **konkretny, pokazany na przykładach i regularnie aktualizowany**, gdy widzisz, że jakiś element przestał pasować do marki.
